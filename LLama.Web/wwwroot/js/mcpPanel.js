...
function currentSource() {      
    return [...document.querySelectorAll('input[name="source"]')].find(r => r.checked).value;
}
...
async function callMcp(req, onChunk) {
    ...
    try {
        ...
        if (Array.isArray(parsed?.chunks)) {
            parsed.chunks.forEach(t => onChunk(String(t)));
...
async function runOnce(overrides = {}) {
    ...
    try {
        ...
        const raw = source === "local"
            ? await streamLocal(req, (c) => appendOutputChunk(c))
            : await callMcp(req, (c) => appendOutputChunk(c));
...
function refreshScenarioList() {
    ...
    Object.keys(localStorage).filter(k => k.startsWith(SC_PREFIX)).sort().forEach(k => {
...
async function simulate() {
    ...
    try {
        // Run in parallel using the current source (Local or MCP)
